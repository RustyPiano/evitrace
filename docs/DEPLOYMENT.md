...
- **文本解析分块**：TXT/MD/DOCX/PDF 会采用目标尺寸语义分块，连续短段落合并为富块，单个超长段落按换行或句末边界优先硬切；`PARSE_CHUNK_TARGET_CHARS`（默认 800，100..8000）控制合并目标，`PARSE_CHUNK_MAX_CHARS`（默认 1600，200..16000）控制硬切上限。相比旧版“每段/每 1000 字一条”，这会大幅减少证据条数、提升每条语义完整度，并与去重、相关性预筛、批量字符上限和限流冷却协同降低大语料批数与成本。
